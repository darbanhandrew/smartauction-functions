...
async function sendSms(KAVENEGAR_URL, KAVENEGAR_API_TOKEN,phoneNumber,fullName, auctionArtName) {
  return new Promise((resolve, reject) => {
    https.get(`${KAVENEGAR_URL}${KAVENEGAR_API_TOKEN}/verify/lookup.json?receptor=${phoneNumber}&token10=${fullName}&token=${auctionArtName}&template=smartauctionfinish`, (response) => {
      resolve();
    }).on('error', (error) => {
      reject(error);
    });
